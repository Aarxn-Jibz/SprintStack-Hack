...
export type RouteEvaluation = { order: string[]; distanceMeters: number; travelDurationSeconds: number; waitingDurationSeconds: number; serviceDurationSeconds: number; totalDurationSeconds: number; feasible: boolean; stops: RouteStop[]; violations: Violation[]; endTimeSeconds: number; load: number };
export type TripEvaluation = RouteEvaluation & { trip: number };
export type UnassignedStop = { id: string; reason: "demand-exceeds-capacity" | "time-window-infeasible" | "unreachable" };
