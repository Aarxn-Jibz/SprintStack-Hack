...
export async function optimizeRoute(request: OptimizeRequest, provider: RoutingProvider) {
  ...
  const optimized = combine(trips); const lines: number[][][] = [];
  for (const trip of trips) { const tripPoints = [request.depot, ...trip.order.map(id => request.stops.find(stop => stop.id === id)!), request.depot]; let geometry = straightLineGeometry(tripPoints); if (!routing.degraded) try { geometry = await provider.getRouteGeometry(tripPoints); } catch { /* straight lines remain usable */ } lines.push(geometry.geojson.geometry.coordinates); }
